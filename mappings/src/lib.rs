...
mod enabled {
    ...
    use libc::{
        ...
        _dyld_image_count, LC_SEGMENT_64, MH_MAGIC_64, VM_PROT_EXECUTE, VM_PROT_READ, load_command,
        mach_header_64, segment_command_64,
    };

    use pprof_util::{BuildId, CastFrom};

    use crate::{LoadedSegment, SharedObject, assert_pointer_valid};
    ...
    /// to symbolize stack traces offline.
    ///
    /// Uses dyld's image APIs to walk all loaded Mach-O images and extract the
    /// wanted information from their load commands.
    ///
    /// SAFETY: This function follows dyld-provided pointers to loaded image
    /// headers and interprets them as Mach-O structures. The only known safety
    /// requirement is that the running binary must be 64-bit Mach-O on macOS.
    pub unsafe fn collect_shared_objects() -> Result<Vec<SharedObject>, anyhow::Error> {
    ...
    mod tests {
        use libc::{
            VM_PROT_EXECUTE, VM_PROT_NONE, VM_PROT_READ, VM_PROT_WRITE, segment_command_64,
        };
